<template>
    <div class="board">
      <div
        ...
        :class="['square', 
                square.color,
                { hightlight: square.highlight,
                selected: square === selectedSquare }]"
        @click="onSquareClick(square)"
        @dragstart="onDragStart(square)"
        @drop="onDrop(square)"
        @dragover.prevent
      >
        <img v-if="square.piece" :src="getPieceImage(square.piece)" :alt="square.piece" class="piece-image" draggable="true" />
      </div>
    </div>
  ...
  export default {
    ...
    isEnnemyPiece(square) {
      return (this.whiteToMove ^ square.whitePiece)
    },

    async onSquareClick(square) {
      if (this.isEnnemyPiece(square)) {
        return;
      } 
    ...
    async onDrop(square) {
        if (this.selectedSquare && this.selectedSquare !== square) {
          await this.makeMove(this.selectedSquare, square.index);
          this.selectedSquare = null;
          this.clearHighlights();
        }
    },
    ...
    highlightAvailableMoves(square) {
      this.clearHighlights();
        for (const move of square.availableMoves) {
          this.board[move].highlight = true;
        }
    },
    ...
    async makeMove(sourceIndex, destIndex) {
      const move = (sourceIndex & 0x3F) | ((destIndex & 0x3F) << 6);
      try {
        await makeMove(move);
    ...
    async updateBoard() {
      try {
        ...
        for (const move of moves) {
          this.board[move.source].availableMoves.push(move.dest);
        }
